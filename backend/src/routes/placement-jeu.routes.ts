...
export default async function placementJeuRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/', { preHandler: fastify.authenticate }, placementJeuController.create);

  // PUT /api/placements-jeux/:id - Met à jour un placement de jeu
  fastify.put('/:id', { preHandler: fastify.authenticate }, placementJeuController.update);

  // DELETE /api/placements-jeux/:id - Supprime un placement de jeu
  fastify.delete('/:id', { preHandler: fastify.authenticate }, placementJeuController.delete);
}
