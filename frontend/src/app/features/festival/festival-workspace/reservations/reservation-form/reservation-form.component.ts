...
import { Component, DestroyRef, EventEmitter, OnInit, Output, effect, inject, signal } from '@angular/core';
...
import { StatutWorkflow } from '../../../../../core/models/reservation.model';
...
import { AutocompleteSearchBarComponent, AutocompleteSearchOption } from '../../../../../shared/components/autocomplete-search-bar/autocomplete-search-bar.component';
...
export class ReservationFormComponent implements OnInit {
  ...
  constructor(
    ...
    effect(() => {
      ...
      if (festivalId) {
        this.loadClassesTarifaires(festivalId);
      }
    });
  }
  ...
  private loadClassesTarifaires(festivalId: number): void {
    this.isLoading.set(true);
    this.classeTarifaireService.getByFestival(festivalId)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (classes) => {
          this.classesTarifaires.set(classes);
          this.ensureClasseSelectionDefaults();
          this.isLoading.set(false);
        },
        error: () => {
          this.errorMessage.set('Impossible de charger les classes tarifaires.');
          this.isLoading.set(false);
        }
      });
  }
  ...
  addReservationClasse(): void {
    ...
    if (!firstAvailableClasseId && this.classesTarifaires().length > 0) {
  ...
  getSelectedTablesTotal(): number {
    return this.reservationClasses.controls.reduce((total, control) => {
      ...
      const idClasseTarifaire = Number(value.idClasseTarifaire);
      const nbTables = Number(value.nbTables);
      if (idClasseTarifaire <= 0 || nbTables <= 0) return total;
  ...
  onSubmit(): void {
    ...
    if (!festivalId || !selectedEditeur) {
      this.errorMessage.set('Veuillez sélectionner un éditeur.');
      return;
    }
  ...
  private getUsedClasseIds(excludedRowIndex?: number): Set<number> {
    ...
    this.reservationClasses.controls.forEach((control, index) => {
      if (excludedRowIndex !== undefined && index === excludedRowIndex) return;
      const value = control.getRawValue() as { idClasseTarifaire: number | string };
      const idClasseTarifaire = Number(value.idClasseTarifaire);
      if (idClasseTarifaire > 0) ids.add(idClasseTarifaire);
    });
    return ids;
  }
